...
cin.ignore();
getline(cin, newBook.title);cout << "Enter the book author: ";
...
cout << "Availability: " << (books[i].available ?
"Available" : "Issued") << endl;
found = true;
break;
}
}
...
cin >> bookID;
bool found = false;for (int i = 0; i < count; i++) {
...
cout << "Book Title: " << books[i].title << endl;
cout << "Book Author: " << books[i].author << endl;
cout << "Availability: " << (books[i].available ? "Available" :
"Issued") << endl;
cout << endl;
}
}
...
cin >> bookID;bool found = false;
for (int i = 0; i < count; i++) {
if (books[i].id == bookID) {
// Move the last book to the current position to "delete"
...
cout << "5. Return a Book" << endl;cout << "6. List All Books" << endl;
